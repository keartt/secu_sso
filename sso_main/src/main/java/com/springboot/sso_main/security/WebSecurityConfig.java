...
public class WebSecurityConfig  extends WebSecurityConfigurerAdapter {
    ...
    @Bean
    public JustFilter justFilter() throws Exception {
        return new JustFilter(this.authenticationManager());
    }
    ...
    protected void configure(HttpSecurity http) throws Exception {
        http.csrf().disable()
                ...
                // 로그인 페이지 설정
                .formLogin()
                    .loginPage("/login")               // 로그인 폼 보여줄 페이지
                    .loginProcessingUrl("/login")          // 실제 로그인 처리 요청 URL
                    .defaultSuccessUrl("/", true)      // 로그인 성공 시 이동할 URL
                    .failureUrl("/login")   // 실패 시 다시 로그인 페이지로
                    .permitAll()
                .and()
                .logout()
        ...
        final JustFilter filter = justFilter();
